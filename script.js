...
/*/* =======================================================================
   AHS — MOUSE PARTICLES (v2: brighter + higher layer + ignores reduce-motion if you want)
   ======================================================================= */
(function AHS_mouseParticles(){
  // If you want to respect reduce-motion, set RESPECT_RM = true
  const RESPECT_RM = false; // set to true later if you want to honor OS setting
  if (RESPECT_RM && window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
  ...
  Object.assign(canvas.style, {
    position: "fixed",
    inset: "0",
    zIndex: "5",              // higher so it’s clearly above starfield and backgrounds, but still under content
    pointerEvents: "none",
    mixBlendMode: "plus-lighter" // brighter than 'screen' on most browsers
  });
  ...
  function make(x, y){
    ...
    return {
      ...
      life: 1,
      hue: 205 + Math.random()*35 // blue range
    };
  }
  ...
  addEventListener("pointermove", e => {
    ...
    spawn(mouseX, mouseY, TRAIL);
  }, { passive: true });

  // gentle idle so there’s always a hint
  setInterval(()=> spawn(mouseX, mouseY, 2), 500);
  ...
  (function tick(){
    ...
    for (let i=particles.length-1; i>=0; i--){
      ...
      if (p.life <= 0 || p.x < -20 || p.x > innerWidth+20 || p.y > innerHeight+20){
        particles.splice(i,1);
        continue;
      }

      const alpha = Math.max(0, p.life) * (themeIsLight() ? 0.95 : 0.75);
      ctx.beginPath();
      ctx.arc(p.x, p.y, p.r, 0, Math.PI*2);
      ctx.fillStyle = `hsla(${p.hue}, 90%, ${themeIsLight()? 40 : 70}%, ${alpha})`;
      ctx.shadowBlur = 14;
